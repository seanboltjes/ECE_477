...
bool IMU_9DOF::GetQuaternion(Quaternion& quaternion)
{
    ...
    uint32_t now = micros();
	deltat = ((now - microsSinceLastTimeValuesUpdated) / 1000.0f); // set integration time by time elapsed since last filter update
	microsSinceLastTimeValuesUpdated = now;
    ...
    ComputeQuaternion(quaternion, accel, gyro, magnetometer);
}
